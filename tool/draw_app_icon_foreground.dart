...
/// Generates Android adaptive foreground icon layer:
...
void main() {
  const size = 1024;
  const dark = 0xFF1C1C1E;
  const white = 0xFFFFFFFF;

  final image = img.Image(width: size, height: size, numChannels: 4);
  img.fill(image, color: _rgba(0, 0, 0, 0)); // transparent background

  final cx = size ~/ 2;
  final cy = size ~/ 2;

  // Calendar symbol (without circular background).
  final calW = size * 0.5;
  ...
  final darkColor = _rgba((dark >> 16) & 0xFF, (dark >> 8) & 0xFF, dark & 0xFF, 255);
  ...
  // Top bar.
  img.fillRect(image, x1: left, y1: top, x2: left + w, y2: top + barH, color: darkColor);
  ...
  for (int row = 0; row < 3; row++) {
    for (int col = 0; col < 4; col++) {
      ...
      img.fillCircle(image, x: px.round(), y: py.round(), radius: dotR, color: darkColor);
    }
  }

  final path = 'assets/icon/app_icon_foreground.png';
